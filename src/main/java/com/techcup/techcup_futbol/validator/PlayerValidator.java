...
public class PlayerValidator {
    ...
    public static void validateEmailDomain(String email) {
        ...
        if (!lowerEmail.endsWith(SCHOOL_DOMAIN) && !lowerEmail.endsWith(GMAIL_DOMAIN)) {
            throw new IllegalArgumentException("Email must belong to @escuelaing.edu.co or @gmail.com");
        }
    }

    /**
     * Valida que el número de identificación no esté repetido en el DataStore.
     */
    public static void validateUniqueness(Integer numberID) {
        boolean exists = DataStore.jugadores.values().stream()
                .anyMatch(p -> p.getNumberID() == numberID);

        if (exists) {
            throw new IllegalArgumentException("A player with this ID number is already registered.");
        }
    }

    /**
     * Valida que el dorsal sea único dentro de un equipo (Lógica avanzada para más adelante)
     */
    public static void validateDorsal(int dorsal) {
        if (dorsal < 1 || dorsal > 99) {
            throw new IllegalArgumentException("Dorsal number must be between 1 and 99.");
        }
    }
}
